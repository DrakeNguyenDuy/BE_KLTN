...
public interface MerchantRepository extends JpaRepository<MerchantStore, Integer>, MerchantRepositoryCustom,
		JpaSpecificationExecutor<MerchantStore> {

	@Query("select m from MerchantStore m " + "left join fetch m.parent mp" + "left join fetch m.country mc "
			+ "left join fetch m.currency mc " + "left join fetch m.zone mz " + "left join fetch m.defaultLanguage md "
	...
	@Query(value = "select * from {h-schema}MERCHANT_STORE m "
			+ "where m.STORE_CODE = ?1 or ?2 is null or m.PARENT_ID = ?2", nativeQuery = true)
