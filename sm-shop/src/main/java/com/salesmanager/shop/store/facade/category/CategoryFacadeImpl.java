...
public class CategoryFacadeImpl implements CategoryFacade {
	...
	public ReadableCategoryList getCategoryHierarchy(MerchantStore store, ListCriteria criteria, int depth,
		...
		try {
			...
			if (!CollectionUtils.isEmpty(filter) && filter.contains(FEATURED_CATEGORY)) {
				...
				org.springframework.data.domain.Page<Category> pageable = categoryService.getListByDepth(parent,
						language, criteria != null ? criteria.getName() : null, depth, page, count);
			...
			if (filter != null && filter.contains(VISIBLE_CATEGORY)) {
				readableCategories = categories.stream().filter(Category::isVisible)
						.map(cat -> readableCategoryMapper.convert(cat, store, language)).collect(Collectors.toList());
			} else {
	...
	public PersistableCategory saveCategory(MerchantStore store, PersistableCategory category) {
		try {
			...
			Category target = Optional.ofNullable(categoryId).filter(merchant -> store != null).filter(id -> id > 0)
					.map(categoryService::getById).orElse(new Category());
	...
	private Category populateCategory(PersistableCategory category, Category target) {
		try {
			return persistableCatagoryPopulator.populate(category, target);
...
//	end
	...
	private void saveCategory(MerchantStore store, Category category, Category parent) throws ServiceException {
		...
		List<Category> saveAfter = children.stream().filter(c -> c.getId() == null || c.getId().longValue() == 0)
				.collect(Collectors.toList());
		List<Category> saveNow = children.stream().filter(c -> c.getId() != null && c.getId().longValue() > 0)
				.collect(Collectors.toList());
	...
	@Override
	public ReadableCategory getCategoryByFriendlyUrl(MerchantStore store, String friendlyUrl, Language language)
			throws Exception {
	...
	public List<ReadableProductVariant> categoryProductVariants(Long categoryId, MerchantStore store,
		...
		try {
			...
			for (ProductAttribute attr : attributes) {
				...
				List<com.salesmanager.shop.model.catalog.product.attribute.ProductOptionValue> values = rawFacet
						.get(attr.getProductOption().getCode());
				if (values == null) {
			...
			while (it.hasNext()) {
				...
				if (optionDescription.isPresent()) {
					...
					List<ReadableProductVariantValue> readableValues = optionValues.stream().sorted(orderComparator)
							.collect(Collectors.toList());
	...
	public ReadableCategoryList listByProduct(MerchantStore store, Long product, Language language) {
		...
		readableCategories = categories.stream().map(cat -> readableCategoryMapper.convert(cat, store, language))
				.collect(Collectors.toList());
	...
	@Override
	public PersistableCategory saveCategory(PersistableCategory category) {
		try {

			Long categoryId = category.getId();
			Category target = Optional.ofNullable(categoryId).filter(id -> id > 0).map(categoryService::getById)
					.orElse(new Category());

			Category dbCategory = populateCategory(category, target);
			categoryService.save(dbCategory);
			// set category id
			category.setId(dbCategory.getId());
			return category;
		} catch (ServiceException e) {
